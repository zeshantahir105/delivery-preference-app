...
├── frontend/                   # React SPA
│   ├── src/
│   │   ├── api/client.ts       # login, me, createOrder, getOrder, updateOrder, getOrderSummary (fetch + token)
│   │   ├── context/AuthContext.tsx  # token/user state, setToken, signOut, me() on mount
│   │   ├── pages/
│   │   │   ├── Login.tsx        # Email/password form, redirect on success
│   │   │   ├── Preference.tsx  # IN_STORE | DELIVERY | CURBSIDE, conditional address/pickup_time
│   │   │   └── Summary.tsx     # Show order, AI summary (Generate/Regenerate), edit order, sign out
│   │   ├── App.tsx             # Routes, ProtectedRoute (redirect to /login if not authenticated)
...
2. **Routes**:
   ...
   - `GET /me`, `POST /orders`, `GET /orders/:id`, `PUT /orders/:id`, `GET /orders/:id/summary` → wrapped with `auth(...)` so JWT is required.
...
| **React Router**             | Routes in `App.tsx`: `/login`, `/` (Preference), `/summary`; `ProtectedRoute` redirects unauthenticated users to `/login`                                                      |
| **React Hook Form + Zod**    | Login and Preference use `useForm` with `zodResolver(schema)`; inline validation errors                                                                                        |
| **localStorage**             | Token stored in `localStorage`; `AuthContext` reads it on load and passes to API client; order id stored for Summary/Edit                                                      |
| **Pages**                    | Login (email/password, redirect on success); Delivery Preference (IN_STORE/DELIVERY/CURBSIDE, conditional fields, future datetime); Summary (show order, edit order, sign out) |
...
3. **API client** (`api/client.ts`):
   ...
   - Exposes: `login`, `me`, `createOrder`, `getOrder`, `updateOrder`, `getOrderSummary(orderId)` (typed with `OrderPreference`, `Order`).
...
4. **Pages**:
   ...
   - **Preference**: Select preference; if DELIVERY/CURBSIDE, show address + datetime-local. Zod validates future pickup_time and required address. On submit: create or update order (if `orderId` in localStorage), then navigate to `/summary`. Loads existing order when editing (getOrder by orderId).
   - **Summary**: Shows current user and order (getOrder by orderId). "Order summary" section: "Generate AI summary" (or "Regenerate summary" after first run) calls `getOrderSummary(orderId)` (backend-proxied; OpenAI or Gemini when key set, else fallback). Displays returned summary text and "Generated with AI" when `source === "ai"`. Buttons: Edit order (navigate to `/`), Sign out (signOut + navigate to `/login`).

5. **Routing and guard** (`App.tsx`):
   - `/login` → Login page.
   - `/` → ProtectedRoute → Preference.
   - `/summary` → ProtectedRoute → Summary.
...
- **Summary.test.tsx**: (1) With mocked getOrder, summary reflects backend order data (order id, preference, address). (2) **AI summary**: With mocked getOrder and getOrderSummary (returns `{ summary: '...', source: 'ai' }`), click "Generate AI summary" → getOrderSummary(orderId) is called, summary text and "Generated with AI" are displayed.
- Vitest + jsdom; `@testing-library/react`, `jest-dom`, `user-event`; mocks for `api/client`.

---

## 4. End-to-End Flow (Backend ↔ Frontend)
...
2. **Login**  
   User submits email/password. Frontend POST /auth/login with JSON body. Backend checks user in DB, bcrypt compare; if OK, returns JWT. Frontend stores token, calls me(), then navigates to `/` (Preference).

3. **Set preference**  
   User picks IN_STORE / DELIVERY / CURBSIDE; if not IN_STORE, fills address and pickup time. Frontend validates (Zod: future datetime, required address). On submit: POST /orders (or PUT /orders/:id if editing) with JSON. Backend validates (enum, conditional fields, future pickup_time), inserts/updates order for `user_id` from JWT, returns order. Frontend stores order id, navigates to `/summary`.

4. **Summary and edit**  
   Summary page GET /orders/:id with Bearer token. Backend ensures order belongs to authenticated user, returns order. User can click "Generate AI summary"; frontend GET /orders/:id/summary with Bearer token. Backend builds order description (order number, preference, address, pickup time, creation date), sends prompt to OpenAI (if OPENAI_API_KEY set) or Gemini (if GEMINI_API_KEY set); returns `{ summary, source }`. Frontend displays summary and "Generated with AI" when source is ai. “Edit order” navigates to `/`; Preference page loads order via GET /orders/:id and pre-fills form; submit does PUT /orders/:id.
...
- Docker-ready: **Yes**.
- Endpoints: POST /auth/login, GET /me, POST /orders, GET /orders/:id, PUT /orders/:id, GET /orders/:id/summary: **Yes** (all implemented; orders scoped by user; summary returns AI or fallback).
...
- Pages: Login (validation, redirect); Delivery Preference (options, conditional fields, future datetime); Summary (display, edit, sign out): **Yes**.
...
- How to run (Docker + local), project features, default user, environment variables, tests, API endpoints, architecture, design decisions, tradeoffs: **Yes** (README + this doc).
